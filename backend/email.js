...
export async function sendConfirmationEmail({ to, event, participant }) {
  ...
  if (mode === 'sendgrid') {
    try {
      const resp = await sgMail.send({ to, from: SENDER_EMAIL, subject, html });
      console.log('[EMAIL] sendgrid ok:', resp[0]?.statusCode);
    } catch (err) {
  ...
  console.warn('[EMAIL] deshabilitado (no hay SendGrid ni SMTP)');
}
